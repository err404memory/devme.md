...
def _load_config() -> dict:
    """Load ~/.devme/config.json, falling back to defaults for any missing key."""
    defaults = {
        "username": "you",
        "filename": "me.md",
...
# Overview file — canonical name first, legacy fallbacks after
OVERVIEW_CANDIDATES = [
    "overview.md",
    "project.md",
    "_OVERVIEW.md",
...
def append_changelog_row(ash_path: Path, event: str, now: datetime = None) -> None:
    """Append a timestamped row to the Change Log table in a companion file."""
...
def collect_parent_links(target_dir: Path) -> list:
    """Walk up toward $HOME collecting (name, path) for each dir with a companion file."""
    home = Path.home()
    links = []
...
def collect_children_links(target_dir: Path) -> list:
    """Scan immediate subdirectories for companion files."""
    links = []
    try:
...
def collect_sibling_links(target_dir: Path) -> list:
    """Scan sibling directories (same parent, excluding self) for companion files."""
    links = []
...
def replace_navigation_section(
    ash_path: Path, nav_lines: list, now: datetime = None
) -> None:
    """Replace the ## Navigation section body in an existing companion file."""
    if not ash_path.exists():
        return
    lines = ash_path.read_text().splitlines()
    ...
    ash_path.write_text("\n".join(lines) + "\n")
    append_changelog_row(ash_path, "Navigation refreshed by `devme update`", now)
...
def _insert_nav_link(
    ash_path: Path, new_line: str, event: str, now: datetime = None
) -> None:
...
def backpropagate_links(
    new_ash: Path, parents: list, siblings: list, now: datetime = None
) -> None:
    """Update nearby companion files to include a link back to the new file."""
...
def parse_overview(dir_path: Path) -> dict:
    """Parse the overview file from dir_path."""
...
def _extract_ash_section(ash_path: Path, section: str) -> str:
    """Return the body text of a ## section in a companion file."""
    in_section = False
    body = []
...
def replace_ash_section(ash_path: Path, section: str, new_body: str) -> None:
    """Replace the body of a ## section in a companion file, preserving the heading."""
    lines = ash_path.read_text().splitlines()
    start = None
    end = None
    for i, line in enumerate(lines):
...
def pull_from_overview(ash_path: Path, now: datetime = None) -> bool:
    """Pull Status and Next Steps from the overview file into a companion file. Returns True if updated."""
    ...
    if updated:
        append_changelog_row(ash_path, "Synced from overview file", now)
...
def push_to_overview(ash_path: Path, now: datetime = None) -> bool:
    """Push Status and Next Steps from a companion file back to the overview file. Returns True if updated."""
    ...
    # Next_action in frontmatter: explicit Status field first, fallback to first Next Steps item
    ...
    if m:
        ...
        if candidate and not candidate.startswith("<!--"):
            next_action_value = candidate

    if not next_action_value:
        for line in next_steps_body.splitlines():
            line = line.strip()
            ...
            line = line.strip()
            if line:
                next_action_value = line
                break

    if next_action_value:
        fm_match = re.match(r"^(---\n)(.*?)(\n---\n?)", ov_content, re.DOTALL)
        if fm_match:
            fm_body = fm_match.group(2)
            if re.search(r"^Next_action:\s*.*$", fm_body, re.MULTILINE):
                fm_body = re.sub(
                    r"^Next_action:\s*.*$",
                ...
                if fm_body and not fm_body.endswith("\n"):
                    fm_body += "\n"
                fm_body += f"Next_action: {next_action_value}"

            ov_content = f"{fm_match.group(1)}{fm_body}{fm_match.group(3)}{ov_content[fm_match.end() :]}"

    if ov_content != original:
        overview.write_text(ov_content)
        append_changelog_row(ash_path, "Pushed to overview file", now)
        return True
...
def find_readme(target_dir: Path) -> Path:
    ...
    )  # non-existent placeholder — clicking creates it in Zed
...
def detect_ai_notes(target_dir: Path, ash_path: Path) -> list:
    ...
    for dirpath_str, dirnames, filenames in os.walk(str(target_dir), topdown=True):
        ...
        for filename in sorted(filenames):
            ...
            if is_ai:
                found.append(
                    {"rel": rel, "path": md_file, "excerpt": _first_heading(md_file)}
                )
    return found
...
def build_template(
    ...
    if overview:
        ...
        status_body = (
            "\n".join(status_lines)
            if status_lines
            else "<!-- What's the current state of this project? -->"
        )
        next_steps_body = overview.get(
            "next_steps", "<!-- What needs to happen next? -->"
        )
        sync_note = f"\n| {ts(now)} | Synced from overview file |"
    else:
...
| Date | Event |
|------|-------|
| {ts(now)} | Created by `devme init` |{sync_note}
...
def cmd_init(args):
    ...
    content = build_template(
        target, parents, children, siblings, ai_notes, now, overview
    )
...
def cmd_update(args):
    ...
    replace_navigation_section(
        ash_path, build_navigation_lines(parents, children, siblings), now
    )
...
def cmd_push(args):
    ...
    overview = find_overview(ash_path.parent)
    if overview is None:
        print(f"Error: no overview file found in {ash_path.parent}", file=sys.stderr)
        sys.exit(1)
    now = datetime.now()
...
def cmd_refresh(_args):
    ...
    for p in paths:
        ...
        siblings = collect_sibling_links(project_dir)
        replace_navigation_section(
            p, build_navigation_lines(parents, children, siblings), now
        )
...
def cmd_upgrade(args):
    """Patch structural elements of existing companion files without touching content."""
    target = Path(args.path).expanduser().resolve()
...
def _upgrade_file(ash_path: Path) -> bool:
    """Apply structural upgrades to a single companion file in-place."""
    now = datetime.now()
    content = ash_path.read_text()
    changed = False
...
def cmd_rename_overview(args):
    """Rename overview files (e.g. overview.md → devme.md) across registered projects."""
    ...
    if renamed and new_name not in OVERVIEW_CANDIDATES:
        print(
            f"\nNote: add '{new_name}' to OVERVIEW_CANDIDATES in ~/.local/bin/devme to find it."
        )
...
def cmd_watch(args):
    """Poll companion file and auto-push to the overview file whenever it's modified."""
    import time
    ...
    try:
        while True:
            ...
            if mtime != last_mtime:
                ...
                now = datetime.now()
                if push_to_overview(ash_path, now):
                    print(f"  [{ts(now)}] Pushed changes to overview file")
                else:
...
def _read_bundled_asset(name: str) -> str | None:
    """Read a packaged asset from the installed devme package."""
    try:
        return resources.files("devme").joinpath(name).read_text(encoding="utf-8")
    except Exception:
        return None


def _read_source_asset(name: str) -> str | None:
    """Read an asset from a source checkout as a development fallback."""
    repo_root = Path(__file__).resolve().parents[2]
    for candidate in (repo_root / name, Path.cwd() / name):
        if candidate.exists():
...
def _read_asset(name: str) -> str | None:
    """Read an asset from package resources, then source checkout fallback."""
    bundled = _read_bundled_asset(name)
    if bundled is not None:
        return bundled
    return _read_source_asset(name)
...
def ensure_runtime_assets(force: bool = False) -> list[Path]:
    ...
    if missing:
        print(
            "Error: unable to load bundled assets: " + ", ".join(missing),
            file=sys.stderr,
        )
        print(
            "Reinstall the package and run: devme install --force",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _load_serve_html() -> str:
    """Load serve.html from ~/.devme/, substituting current config values."""
    if not _SERVE_HTML_PATH.exists():
        print(
            f"Error: {_SERVE_HTML_PATH} not found. Re-run devme to restore it.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    html = html.replace(
        "__ICON_FOLDER_OPEN__", cfg.get("icon_folder_open", _svg_folder_open)
    )
    html = html.replace(
        "__ANN_ICON_COLOR__", cfg.get("ann_icon_color", cfg["accent_color"])
    )
    return html


def _load_wizard_html() -> str:
    """Load wizard.html from ~/.devme/."""
...
<p>Run <code>devme install --force</code> to restore bundled setup assets.</p>
</body></html>"""
...
def _run_install(data: dict) -> dict:
    ...
    qs_path.write_text(
        ...
        f"## First steps\n\n"
        f"```sh\n"
        f"devme init ~/projects/my-app   # create a companion file\n"
        f"devme serve                    # open the interface at http://localhost:7272\n"
        f"```\n\n"
        f"On first load, the interface starts a guided tour automatically. Replay it any time with the Tour button in the top bar.\n\n"
        f"## Commands\n\n"
        f"```sh\n"
        f"devme serve                    # start the local interface\n"
        f"devme install                  # re-run setup wizard\n"
        f"devme init [path]              # create a companion file in a directory\n"
        f"devme update [path]            # refresh navigation links\n"
        f"devme refresh                  # rebuild navigation in all registered files\n"
        f"devme push [path]              # write Status/Next Steps back to project overview\n"
        f"devme watch [path]             # continuous sync with project overview on save\n"
        f"devme upgrade [--all]          # migrate older companion files to current format\n"
        f"devme rename-overview <name>   # rename project overview files\n"
        f"devme rm [path]                # remove a directory from the mesh index\n"
        f"devme rm [path] --delete       # remove from index and delete the companion file\n"
        f"```\n\n"
        f"## Config: `~/.devme/config.json`\n\n"
        f"Edit this file to change any setting. Re-run `devme install` to use the wizard again.\n\n"
        f"## Docs\n\n"
    ...
    class AshHandler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if p == "/":
                ...
                try:
                    self._send(
                        200, "text/plain; charset=utf-8", Path(path).read_bytes()
                    )
...
def cmd_serve(args):
    ...
    ensure_runtime_assets(force=False)
...
def cmd_rm(args):
    ...
    if args.delete:
        ...
        print(
            f"Companion file kept at {ash_path}  (use --delete to remove it from disk too)"
        )
...
def cmd_install(args):
    ...
    written = ensure_runtime_assets(force=getattr(args, "force", False))
    for path in written:
        print(f"Installed asset: {path}")
...
def main():
    ...
    install_p.add_argument(
        "--force", action="store_true", help="Re-copy files even if already installed"
    )
    install_p.add_argument(
        "--port", type=int, default=7272, help="Port to serve on (default: 7272)"
    )

    init_p = sub.add_parser("init", help="Initialize a companion file in a directory")
    init_p.add_argument(
        "path", nargs="?", default=".", help="Target directory (default: .)"
    )
    init_p.add_argument(
        "--force", action="store_true", help="Overwrite existing companion file"
    )

    update_p = sub.add_parser(
        "update", help="Refresh navigation + pull from overview file"
    )
    ...
    push_p = sub.add_parser(
        "push", help="Push Status/Next Steps from companion file to overview file"
    )
    push_p.add_argument(
        "path", nargs="?", default=".", help="Target directory (default: .)"
    )
    ...
    upgrade_p = sub.add_parser(
        "upgrade",
        help="Patch structure of existing companion files (tree, collapsible sections)",
    )
    upgrade_p.add_argument(
        "path", nargs="?", default=".", help="Target directory (default: .)"
    )
    upgrade_p.add_argument(
        "--all", action="store_true", help="Upgrade all registered companion files"
    )

    rename_p = sub.add_parser(
        "rename-overview", help="Rename overview files (e.g. overview.md → devme.md)"
    )
    ...
    rename_p.add_argument(
        ...
        nargs="?",
        default=None,
        help="Directory to search (default: all registered projects)",
    )

    watch_p = sub.add_parser(
        "watch", help="Auto-push to overview file whenever companion file is saved"
    )
    ...
    sub.add_parser(
        "refresh", help="Refresh navigation in all registered companion files"
    )
    ...
    rm_p.add_argument(
        ...
        nargs="?",
        default=".",
        help="Directory or companion file path (default: .)",
    )
    rm_p.add_argument(
        "--delete", action="store_true", help="Also delete the companion file from disk"
    )

    serve_p = sub.add_parser(
        "serve", help="Start local web server for the companion file mesh"
    )
